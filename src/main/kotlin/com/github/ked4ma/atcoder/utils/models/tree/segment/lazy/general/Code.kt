package com.github.ked4ma.atcoder.utils.models.tree.segment.lazy.general
...
class GeneralLazySegmentTree<X, M>(
    ...
    companion object {
        inline fun <reified X, reified M> instanceOf(
            ...
            ex: X,
            em: M
        ): GeneralLazySegmentTree<X, M> {
            var x = 1
            while (x < n) {
                x *= 2
            }
            val dat = sizedArray(2 * x, ex)
            val lazy = sizedArray(2 * x, em)
